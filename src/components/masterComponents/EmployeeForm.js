...
const EmployeeForm = () => {
    ...
    const [showIncomeRemoveButton, setShowIncomeRemoveButton] = useState(false);
    const [showDeductionRemoveButton, setShowDeductionRemoveButton] = useState(false);
    ...
    const handleDeductionFormChange = (e, index) => {
        const { value } = e.target;
        const newData = [...deductionFormData];
        newData[index] = value;
        setDeductionFormData(newData);
    };
    ...
    const addIncomeFields = () => {
        ...
        setAddIncomeField(newIncomeField);
        setShowIncomeRemoveButton(true)
    };
    const addDeductionField = () => {
        const newDeductionField = [...deductionField, ...DeductionForm];
        setDeductionField(newDeductionField)
        setShowDeductionRemoveButton(true);
    };
    const handleRemove = (formType) => {
        if (formType === 'income') {
            const remainingFieldsCount = IncomeField.length - IncomeForm.length;
            if (remainingFieldsCount > 0) {
                setAddIncomeField(prevState => prevState.slice(0, -IncomeForm.length));
            }
            setShowIncomeRemoveButton(remainingFieldsCount > 0);
        } else if (formType === 'deduction') {
            const remainingFieldsCount = deductionField.length - DeductionForm.length;
            if (remainingFieldsCount > 0) {
                setDeductionField(prevState => prevState.slice(0, -DeductionForm.length));
            }
            setShowDeductionRemoveButton(remainingFieldsCount > 0);
        }
    };
    ...
    return (
        <>
            <section className="VendorM-manu d-flex flex-column align-items-end  justify-content-end">
                ...
                <div className="col-lg-2 col-md-2 col-sm-3 border profile mb-3 mx-3 ">
                    {selectedFile ? (
                        ...
                        <>
                            <label htmlFor='file-input' className="profilediv align-items-center d-flex justify-content-center flex-column">
                                ...
                                <h5 className="text-center">Upload Profile</h5>
                            </label>
                            ...
                            <Form>
                                ...
                                <div className="col-lg-7 col-md-12 col-sm-12">
                                    ...
                                    <div className="d-flex siteform">
                                        <Form.Group as={Col} lg={6} md={6} sm={3} >
                                        ...
                                        <Form.Group as={Col} lg={6} md={6} sm={3} className="px-3" >
                                    ...
                                    <div className="mt-5">
                                        ...
                                        <div className="d-flex row">
                                                ...
                                                <React.Fragment key={index}>
                                                    <Form.Group as={Col} lg={4} md={6} sm={12} className="mb-3">
                                                        {field.options && field.options.length > 0 ? (
                                                            ...
                                                            <Form.Control type="text" value={monthlyFormData[index] || ''}
                                                                onChange={(e) => handleMonthlyFormChange(e, index)} placeholder={field.placeholder} name={field.name} className="" />
                                                        )}
                                                    </Form.Group>
                                            ...
                                            <div className="d-flex col-lg-3 col-md-6 col-sm-6">
                                                ...
                                                {showIncomeRemoveButton && (
                                                    <button className='btn border my-2 ms-2 col-2' type='button' onClick={() => handleRemove('income')}>
                                                        -
                                                    </button>
                                                )}
                                                <button className='btn border m-2' type='button' onClick={handleSalaryAdd}
                                                // disabled={!allIncomeFieldsEntered}
                                                >
                                                    Add
                                                </button>
                                            </div>
                                        ...
                                        <div className="d-flex row ">
                                            {deductionField.map((field, index) => (
                                                <Form.Group as={Col} lg={4} md={6} sm={12} className="mb-3" key={index}>
                                                    {field.options && field.options.length > 0 ? (
                                                        <Form.Select defaultValue="Choose..." value={deductionFormData[index] || ''}
                                                            onChange={(e) => handleDeductionFormChange(e, index)} placeholder={field.placeholder}>
                                                            {field.options.map((option, idx) => (
                                                                <option key={idx}>{option}</option>
                                                            ))}
                                                        </Form.Select>
                                                    ) : (
                                                        <Form.Control type="text" value={deductionFormData[index] || ''}
                                                            onChange={(e) => handleDeductionFormChange(e, index)} placeholder={field.placeholder} className="" />
                                                    )}
                                                </Form.Group>
                                            ))}</div>
                                        <div className="d-flex col-lg-3 col-md-6 col-sm-6">
                                            <button className='btn border my-2 col-2' type='button' onClick={addDeductionField}>
                                                +
                                            </button>
                                            {showDeductionRemoveButton && (
                                                <button className='btn border my-2 ms-2 col-2' type='button' onClick={() => handleRemove('deduction')}>
                                                    -
                                                </button>
                                            )}
                                            <button className='btn border m-2' type='button' onClick={handleDeductionAdd}  >
                                                Add
                                            </button>
                                        </div>
                                    </div>
